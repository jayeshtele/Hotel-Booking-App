...
export default function SearchBar({ compact = false }) {
  ...
  return (
    <form
      ...
      className={clsx(
        'grid gap-3 rounded-[8px] bg-white p-3 shadow-soft',
        compact
          ? 'border border-ink-100 lg:grid-cols-[1.4fr_1fr_1fr_.8fr_auto]'
          : 'lg:grid-cols-[1.5fr_1fr_1fr_.8fr_auto]',
      )}
      data-testid="search-bar"
    >
      <label className="group flex items-center gap-3 rounded-[8px] border border-ink-100 px-4 py-3 transition focus-within:border-ocean-500 focus-within:ring-4 focus-within:ring-ocean-100">
